                                ...
                                long *priority)
{
  ...
  if (configure_srb) {
    ...
    for (int j = 0; j < ue_context_mastercellGroup->rlc_BearerToAddModList->list.count; j++) {
      if (rlc_BearerConfig->logicalChannelIdentity
          == ue_context_mastercellGroup->rlc_BearerToAddModList->list.array[j]->logicalChannelIdentity) {
  ...
  for (int i = bearer_id_start; i < bearer_id_start + nb_bearers_to_setup; i++) {
    ...
    NR_RLC_BearerConfig_t *rlc_BearerConfig = get_DRB_RLC_BearerConfig(3 + i, i, rlc_conf, priority[0]);
    asn1cSeqAdd(&cellGroupConfig->rlc_BearerToAddModList->list, rlc_BearerConfig);
