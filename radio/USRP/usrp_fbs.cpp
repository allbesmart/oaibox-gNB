...
static uhd::time_spec_t start_time;
static uhd::time_spec_t usrp_span_time; //1.3ms at least after set_time_now
static const uhd::time_spec_t expect_time = uhd::time_spec_t(2e-7);
static const uhd::time_spec_t mode_cal_time = uhd::time_spec_t(2e-7);
...
/*
 * Let user calls this function first to sync/get time from usrp, and estimate the next expected execute time
 */
void usrp_sync_time(void)
{
    // printf("usrp_sync_time\r\n");
    start_time = usrp->get_time_now();
    // next expected time after usrp_sync_time()
    start_time = start_time + usrp_span_time;
}
...
// void usrp_set_mode(int mode, openair0_timestamp start_time)
{
    start_time = start_time + expect_time + mode_cal_time;
    ...
    if (debug) {
        start_time = start_time + uhd::time_spec_t(2e-4);  //add more 200us for debug msg
    ...
    start_time = start_time + expect_time;
    usrp->set_command_time(start_time);
...
int usrp_spi_setup(uhd::usrp::multi_usrp::sptr exist_usrp)
{
    ...
    for (size_t i = 0; i < 1; i++){
        ...
        usrp_span_time += (t1-t0);
    }
    std::cout << boost::format(
        " Difference between paired reads: %f us"
    ) % (usrp_span_time.get_real_secs()*1e6) << std::endl;

    // Set to 4ms will be safe for executing on-time
    usrp_span_time = uhd::time_spec_t(4e-4);//4e-3);
    std::cout << boost::format(
        "Set span time to: %f us"
    ) % (usrp_span_time.get_real_secs()*1e6) << std::endl;
