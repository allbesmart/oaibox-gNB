...
//  nr_rrc_mac_config_req_ue(0,0,0,rrc.carrier.mib.message.choice.mib, NULL, NULL, secondaryCellGroup);
  ...
  for (SNR = snr0; SNR <= snr1; SNR += snr_step) {
    ...
    for (trial = 0; trial < n_trials; trial++) {
      ...
      sum_pusch_delay += gNB->measurements.pusch_est_delay[UE_id];
      min_pusch_delay = gNB->measurements.pusch_est_delay[UE_id] < min_pusch_delay ? gNB->measurements.pusch_est_delay[UE_id] : min_pusch_delay;
      max_pusch_delay = gNB->measurements.pusch_est_delay[UE_id] > max_pusch_delay ? gNB->measurements.pusch_est_delay[UE_id] : max_pusch_delay;
